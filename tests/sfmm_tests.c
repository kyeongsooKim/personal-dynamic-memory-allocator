...
//we will check allocation with no splitting case.
Test(sf_memsuite_student, alloc_no_splitting, .init = sf_mem_init, .fini = sf_mem_fini) {

    sf_errno = 0;
	double *x = sf_malloc(PAGE_SZ-32); //will make splinter, so there should be no free block at all.
	...
	*x = 320320320e-320;
	...
	cr_assert(*x == 320320320e-320, "sf_malloc failed to give proper space for an double!");

	/* There should be no free block since it didn't split the free block not to make splinter */
...
//when the request size is greater than the amount of memory the allocator has access to,
//we should set sf_errno to EINVAL and return NULL.
Test(sf_memsuite_student, malloc_invalid_request, .init = sf_mem_init, .fini = sf_mem_fini) {
	sf_errno = 0;
	void *x = sf_malloc(PAGE_SZ * 10);
...
//we will check if coalescging with lower memory would be done correctly when sbrk() is called
Test(sf_memsuite_student, sbrk_lower_mem_coalescing, .init = sf_mem_init, .fini = sf_mem_fini) {


    sf_errno = 0;
    double* x = sf_malloc(PAGE_SZ-48); //will make free block whose size is 32
	cr_assert_not_null(x);

    //requested size is 64, so we call sbrk() one more
    //and should do coalescing with lower memory.
    double* y = sf_malloc(8*sizeof(double));
    ...
    *y = 320320320e-320;
	cr_assert(*y == 320320320e-320, "sf_malloc failed to give proper space for an double!");


	//if coalescing with lower memory is successfuly done when sf_sbrk() is called,
	//There should be no small free block(size 32) in seg_free_lists[1].
	cr_assert_null(seg_free_list[0].head, "There should be no free block(size 32) in seg_free_list[0]");

	//header of x should be lower than get_heap_start() + PAGE_SZ
	//if coalescing with lower memory is successfuly done.
	sf_header *header = (sf_header*)((char*)y - 8);
	cr_assert( header < (sf_header*)(get_heap_start() + PAGE_SZ));

	cr_assert(sf_errno == 0, "sf_errno is not zero!");
	cr_assert(get_heap_start() + 2*PAGE_SZ == get_heap_end(), "Allocated more than necessary!");
}



//Given test (realloc_larger_block) is testing when actual alloc function is called
//when realloc requests larger size, but there is also case when larger size is requested,
//but actual block size is same, in that case, we change only requested_size in footer
Test(sf_memsuite_student, realloc_larger_block_but_same_blocksize, .init = sf_mem_init, .fini = sf_mem_fini) {
	...
	cr_assert_not_null(fl->head, "No block in expected free list!");
	cr_assert_null(fl->head->next, "Found more blocks than expected!");
	cr_assert(fl->head->header.block_size << 4 == PAGE_SZ - (header->block_size << 4),
			 "Free Block size not what was expected!");
	cr_assert(fl->head->header.allocated == 0);


	x = sf_realloc(x, sizeof(int) * 3);

	/* Still There should be one free of with the same size*/
	free_list *fl2 = &seg_free_list[3];
	cr_assert(fl->head->header.block_size == fl2->head->header.block_size, "there should be no size change in free block");

	cr_assert(get_heap_start() + PAGE_SZ == get_heap_end(), "Allocated more than necessary!");

}

//When malloc is called with size 0, set sf_errno as EINVAL and return NULL
//In addition to this, in this test case, since there is only one malloc request with size 0
//heap_start and heap_end should be same.
Test(sf_memsuite_student, malloc_zero_size, .init = sf_mem_init, .fini = sf_mem_fini) {
	sf_errno = 0;
	void *x = sf_malloc(0);

	cr_assert(get_heap_start() == get_heap_end(), "There should be no sbrk call");
	cr_assert_null(x, "x is not NULL!");
	cr_assert(sf_errno == EINVAL, "sf_errno is not EINVAL!");
}
...
//addtional tests function for personal checking. you don't need to grade this test function below although it works correctly.
/*

//given 3 tests(freelist, free_coalesce, free_no_coalesce) do not check footer,
//so we will check footer allocated bit is set correctly when free is called
